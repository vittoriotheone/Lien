...
const TU_PREFERES = [
  ...
  ["Tu préfères les longs câlins silencieux ou les discussions animées jusque tard dans la nuit ?", ""],
  ["Tu préfères les préliminaires longs et sensuels ou aller plus directement au but ?", ""],
  ["Tu préfères les baisers passionnés en public ou les gestes tendres discrets ?", ""],
  ["Tu préfères recevoir un massage sensuel ou en donner un ?", ""],
  ["Tu préfères essayer de nouvelles choses au lit ou refaire ce que vous savez qui marche bien ?", ""],
  ["Tu préfères parler ouvertement de vos fantasmes ou garder une part de mystère ?", ""],
  ["Tu préfères faire l'amour avec la lumière tamisée ou dans le noir complet ?", ""],
  ["Tu préfères les mots doux chuchotés à l'oreille ou les compliments plus directs ?", ""],
  ["Tu préfères une douche ou un bain romantique à deux ?", ""],
  ["Tu préfères surprendre ton/ta partenaire avec de la lingerie/un geste sexy ou être surpris(e) ?", ""],
  ["Tu préfères les câlins du matin au réveil ou ceux du soir avant de dormir ?", ""],
  ["Tu préfères tenir la main en marchant dans la rue ou passer un bras autour de la taille/des épaules ?", ""],
  ["Tu préfères les petits cadeaux coquins ou les expériences sensuelles partagées ?", ""],
  ["Tu préfères te rappeler votre premier baiser ou votre premier \"je t'aime\" ?", ""],
  ["Tu préfères l'imprévu et la spontanéité au lit ou une ambiance préparée (bougies, musique...) ?", ""],
...
const ACTION_VERITE = [
  {type:"verite", text:"Quelle est ta première impression de l'autre ?"},
  {type:"verite", text:"Quelle qualité admires-tu le plus chez l'autre ?"},
  {type:"verite", text:"Quel souvenir de nous te fait le plus sourire ?"},
  {type:"verite", text:"Quelle chose aimerais-tu qu'on fasse plus souvent ensemble ?"},
  {type:"verite", text:"Quel est ton plus grand rêve pour notre avenir ?"},
  {type:"verite", text:"Quelle habitude de l'autre trouves-tu la plus mignonne ?"},
  {type:"verite", text:"Quelle chanson te fait immédiatement penser à l'autre ?"},
  {type:"verite", text:"Quel moment de notre histoire voudrais-tu revivre ?"},
  {type:"verite", text:"Quelle petite attention te touche le plus ?"},
  {type:"verite", text:"Quelle est une chose que tu n'oses pas toujours demander ?"},
  {type:"verite", text:"Quel fou rire partagé retiens-tu le mieux ?"},
  {type:"verite", text:"Quelle activité aimerais-tu absolument essayer à deux ?"},
  {type:"verite", text:"Quelle première impression avais-tu de moi ?"},
  {type:"verite", text:"Quelle chose te fait te sentir vraiment compris(e) ?"},
  {type:"verite", text:"Quel compliment de l'autre t'a le plus marqué ?"},
  {type:"verite", text:"Quelle petite chose pourrais-tu améliorer dans notre quotidien ?"},
  {type:"verite", text:"Quel voyage rêverais-tu de faire avec moi ?"},
  {type:"verite", text:"Quel souvenir d'enfance aimerais-tu me raconter ?"},
  {type:"verite", text:"Quelle tradition aimerais-tu créer avec moi ?"},
  {type:"verite", text:"Quelle est la chose la plus drôle que tu aies faite pour me faire rire ?"},
  {type:"action", text:"Regarde l'autre dans les yeux pendant 30 secondes sans parler."},
  {type:"action", text:"Fais trois compliments sincères à l'autre sans répéter un seul mot-clé."},
  {type:"action", text:"Imite l'autre pendant 30 secondes et laisse-le/la deviner."},
  {type:"action", text:"Chante le refrain d'une chanson choisie par l'autre."},
  {type:"action", text:"Invente un surnom drôle et mignon pour l'autre."},
  {type:"action", text:"Raconte votre meilleur souvenir en exactement cinq phrases."},
  {type:"action", text:"Fais une photo de vous deux qui représente votre duo aujourd'hui."},
  {type:"action", text:"Écris en une minute un mini-poème sur votre duo."},
  {type:"action", text:"Fais rire l'autre en moins de trente secondes."},
  {type:"action", text:"Décris l'autre comme si tu présentais un personnage de film."},
  {type:"action", text:"Rejoue votre première rencontre pendant une minute."},
  {type:"action", text:"Invente une poignée de main secrète à deux."},
  {type:"action", text:"Fais deviner une chanson uniquement en la mimant."},
  {type:"action", text:"Écris une phrase que l'autre pourra garder comme souvenir."},
  {type:"action", text:"Choisis une photo de votre galerie et raconte pourquoi elle compte."},
  {type:"action", text:"Fais un discours de vingt secondes pour célébrer votre duo."},
  {type:"action", text:"Invente une tradition que vous pourriez refaire chaque année."},
  {type:"action", text:"Prépare une mini surprise de moins de cinq minutes."},
  {type:"action", text:"Fais une imitation d'une célébrité et laisse l'autre deviner."},
  {type:"action", text:"Donne à l'autre un compliment sur quelque chose qu'il/elle ne remarque presque jamais."}
];
...
function dedupeGameItems(items) {
  const seen = new Set();
...
function splitTuPrefere(text) {
  const raw = String(text || "").trim();
  const marker = " ou ";
  const idx = raw.toLowerCase().indexOf(marker);
  if (idx > 0) return [raw.slice(0, idx).trim(), raw.slice(idx + marker.length).trim()];
  return [raw, "Choix libre"];
}
function getQuiPool(data) {
  return dedupeGameItems([...QUI_DE_NOUS.map((text,i)=>({key:"base:"+i,text})), ...getCustomByType(data,"qui")]);
}
function getTuPool(data) {
  return dedupeGameItems([
    ...TU_PREFERES.map((pair,i)=>{ const opts = pair[1] ? pair : splitTuPrefere(pair[0]); return {key:"base:"+i,text:opts[0]+" ou "+opts[1],options:opts}; }),
    ...getCustomByType(data,"tu").map(q=>({key:q.key,text:splitTuPrefere(q.text).join(" ou "),options:splitTuPrefere(q.text)}))
  ]);
}
function getAvPool(data, type) {
  ...
  return dedupeGameItems([...base, ...custom]);
}
...
function ensureQui(fresh) {
  ...
  const validCurrent = fresh.qui && pool.some(x => x.key === fresh.qui.currentKey);
  if (!fresh.qui || fresh.qui.version !== 3 || !validCurrent) {
    const used = fresh.qui && Array.isArray(fresh.qui.used) ? fresh.qui.used : [];
    const item = pickUnusedItem(pool, used) || pickUnusedItem(pool, []);
    fresh.qui = { version:3, currentKey:item ? item.key : null, used:item ? [...used, item.key] : used, votes:{A:null,B:null}, roundScored:false, advance:{A:false,B:false} };
...
function ensureTuPrefere(fresh) {
  ...
  const validCurrent = fresh.tuprefere && pool.some(x => x.key === fresh.tuprefere.currentKey);
  if (!fresh.tuprefere || fresh.tuprefere.version !== 3 || !validCurrent) {
    const used = fresh.tuprefere && Array.isArray(fresh.tuprefere.used) ? fresh.tuprefere.used : [];
    const item = pickUnusedItem(pool, used) || pickUnusedItem(pool, []);
    fresh.tuprefere = { version:3, currentKey:item ? item.key : null, used:item ? [...used, item.key] : used, votes:{A:null,B:null}, roundScored:false, advance:{A:false,B:false} };
...
function ensureActionVerite(fresh) {
  if (!fresh.actionVerite || fresh.actionVerite.version !== 4) {
    fresh.actionVerite = { version:4, phase:"choose", currentKey:null, currentText:null, selectedType:null, targetSlot:"A", usedKeys:[], typeHistory:[] };
  } else {
...
function App() {
    ...
    async function refresh() {
        ...
        try {
            ...
            if (data && tab === "jeux" && gameSub === "qui" && data.qui && fresh.qui) {
                const same = data.qui.currentKey === fresh.qui.currentKey;
                if (same) {
                    setData(prev => ({ ...fresh, qui: { ...prev.qui, ...fresh.qui, votes: fresh.qui.votes, advance: fresh.qui.advance } }));
                } else {
                    const localReady = !!(data.qui.votes?.A && data.qui.votes?.B && data.qui.advance?.[mySlot]);
                    setData(prev => localReady ? fresh : ({ ...fresh, qui: prev.qui }));
                }
            } else if (data && tab === "jeux" && gameSub === "tu" && data.tuprefere && fresh.tuprefere) {
                const same = data.tuprefere.currentKey === fresh.tuprefere.currentKey;
                if (same) {
                    setData(prev => ({ ...fresh, tuprefere: { ...prev.tuprefere, ...fresh.tuprefere, votes: fresh.tuprefere.votes, advance: fresh.tuprefere.advance } }));
                } else {
                    const localVotedBoth = (data.tuprefere.votes?.A === 0 || data.tuprefere.votes?.A === 1) && (data.tuprefere.votes?.B === 0 || data.tuprefere.votes?.B === 1);
                    const localReady = localVotedBoth && !!data.tuprefere.advance?.[mySlot];
                    setData(prev => localReady ? fresh : ({ ...fresh, tuprefere: prev.tuprefere }));
                }
            } else if (data && tab === "jeux" && gameSub === "av" && data.actionVerite && fresh.actionVerite) {
                setData(fresh);
            } else {
                setData(fresh);
            }
        } catch (e) {
            console.warn("Synchronisation échouée:", e);
    ...
    async function mutate(fn) {
        ...
        try {
            ...
            const base = remote ? JSON.parse(JSON.stringify(remote)) : JSON.parse(JSON.stringify(data));
            const updated = fn(base);
            if (!updated || typeof updated !== "object") throw new Error("Mutation invalide");
...
function chooseActionVerite(type) {
  if (!data || !data.actionVerite || mySlot !== data.actionVerite.targetSlot) return;
  mutate((fresh) => {
...
function nextActionVerite() {
  mutate((fresh) => {
    const av = ensureActionVerite(fresh);
    if (av.phase !== "question") return fresh;
    av.phase = "choose";
    av.currentKey = null;
    av.currentText = null;
...
function renderActionVerite() {
  ...
  const blockTruth = lastTwo.length >= 2 && lastTwo.every(t => t === "verite");
    if (av.phase === "choose") {
    return React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:16,padding:"4px 4px 20px"}},
      ...
      React.createElement("div",{style:{display:"flex",gap:10}},
        ...
        React.createElement("button",{className:"duo-btn-ghost",disabled:blockTruth,onClick:()=>chooseActionVerite("verite"),style:{flex:1,opacity:blockTruth?.45:1}},"VÉRITÉ")),
