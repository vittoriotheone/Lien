...
import {
  Home, MessageCircle, Shuffle, Image as ImageIcon, Flame, Copy, Check,
  RefreshCw, Sparkles, Heart, ArrowRight, Users, Send, Plus, X,
} from "lucide-react";
...
async function getCouple(codeVal) {
  try {
    const res = await window.storage.get("couple:" + codeVal, true);
...
async function setCouple(codeVal, obj) {
  const res = await window.storage.set("couple:" + codeVal, JSON.stringify(obj), true);
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 16, padding: "4px 20px 20px" }}>
        ...
        <div style={{ fontSize: 11, color: "#A79BC0", textAlign: "center", lineHeight: 1.5, marginTop: 4 }}>
          Vos réponses sont sauvegardées automatiquement et partagées avec {partnerName || "ton/ta partenaire"} via le code {code}.
        </div>
      </div>
    );
  }
